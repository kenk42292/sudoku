...
class Grid {
    ...
    static Cell[][] createGrid(String stringRep) {
        System.out.println("stringRep length: " + stringRep.length());
        Cell[][] grid = new Cell[9][9];
        String[] stringArr = stringRep.trim().split("");
        stringArr = Arrays.copyOfRange(stringArr, 0, stringArr.length);
        System.out.println("stringArr length: " + Arrays.toString(stringArr));
        for (int row = 0; row < 9; row++) {
            for (int col = 0; col < 9; col++) {
